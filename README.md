...
1. `wget -P ~/.config/nvim/fak/packer.lua https://raw.githubusercontent.com/Farooq-azam-khan/nvim-config/main/lua/fak/packer.lua`
2. `vim ~/.config/nvim/lua/fak/packer.lua` and type `:so` in the vim file to source packer deps
3. `rm -r ~/.config/nvim` delete file
4. `git clone https://github.com/Farooq-azam-khan/nvim-config.git ~/.config/nvim`2.
5. `:PackerSync` in the same file
